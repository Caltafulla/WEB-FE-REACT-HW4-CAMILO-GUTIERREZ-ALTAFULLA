const CompStatus = (compliance) => {
  if (compliance <= 35) return { text: "Crítico", color: "red" };
  if (compliance > 35 && compliance <= 75)
...
const OsTable = ({ data }) => {
  return (
    <table style={{ width: "100%" }} border="1" cellPadding="10">
      ...
      <tbody>
        {data.map((objective) => {
          const status = CompStatus(objective.compliance);
          return (
            <tr key={objective.id}>
              ...
              <td className="p-4 text-center" style={{ color: status.color }}>
                {status.text}
              </td>
            </tr>
          );
